...
# and any :( converted to 🙁 (otherwise known as a slightly frowning face). All other text should 
...
# Updated: if text remains unchanged, print out that the text is unchanged

def convert(text):
    if ":)" not in text or ":(" not in text:
        return "text is unchanged"
    else:
        text = text.replace(":)", "🙂")
        text = text.replace(":(", "🙁")
        return text
